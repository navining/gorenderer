...
void CELL::Raster::drawBezier(float2 p0, float2 p3, float2 p1, float2 p2) {
  ...
  for (float t = 0; t < 1.0f; t += 0.01f) {
    float x = p0.x * pow(1 - t, 3) + 3 * p1.x * t * pow(1 - t, 2) +
              3 * p2.x * t * t * (1 - t) + p3.x * t * t * t;
    float y = p0.y * pow(1 - t, 3) + 3 * p1.y * t * pow(1 - t, 2) +
...
void Raster::drawTriangle(int2 p0, int2 p1, int2 p2, Rgba c0, Rgba c1,
                          Rgba c2) {
  if (!isInRect(p0) && !isInRect(p1) && !isInRect(p2)) {
    return;
  }

  Edge edges[] = {Edge(p0.x, p0.y, c0, p1.x, p1.y, c1),
...
void CELL::Raster::drawImage(int startX, int startY, const Image *image, int x1,
                             int y1, int w, int h) {
  if (image == NULL) return;
  int left = tmax<int>(startX, 0);
  int top = tmax<int>(startY, 0);
...
void Raster::drawImageColorKey(int startX, int startY, const Image *image,
                               Rgba key) {
  if (image == NULL) return;
  int left = tmax<int>(startX, 0);
  int top = tmax<int>(startY, 0);
  ...
  for (int x = left; x < right; x++) {
    for (int y = top; y < bottom; y++) {
      Rgba color = image->pixelAt(x - left, y - top);
      if (color != key) setPixelEx(x, y, color);
    }
  }
}

void Raster::drawImageAlphaTest(int startX, int startY, const Image *image,
                                byte alpha) {
  if (image == NULL) return;
  int left = tmax<int>(startX, 0);
  int top = tmax<int>(startY, 0);
  ...
  for (int x = left; x < right; x++) {
    for (int y = top; y < bottom; y++) {
      Rgba color = image->pixelAt(x - left, y - top);
      if (color._a > alpha) setPixelEx(x, y, color);
    }
  }
}
...
void CELL::Raster::drawImageAlpha(int startX, int startY, const Image *image,
                                  float alpha) {
  if (image == NULL) return;
  int left = tmax<int>(startX, 0);
  int top = tmax<int>(startY, 0);
...
void CELL::Raster::drawImageScale(int dstX, int dstY, int dstW, int dstH,
                                  const Image *image) {
...
void CELL::Raster::drawImageScaleHighQuality(int dstX, int dstY, int dstW,
                                             int dstH, const Image *image) {
  float xScale = image->width() / (float)dstW;
  float yScale = image->height() / (float)dstH;
